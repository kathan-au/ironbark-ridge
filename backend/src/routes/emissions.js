...
const { FUEL_TYPE_TO_ACTIVITY, calculateFuelEmissions, calculateElectricityEmissions } = require('../lib/emissionsCalc');
    ...
    try {
       ...
       WHERE cleaned_delivery_date IS NOT NULL AND quantity_litres IS NOT NULL`
        ...
        for (const row of fuelRowsResult.rows) {
            ...
            const emissions = calculateFuelEmissions(parseFloat(row.quantity_litres), factor);
       ...
       WHERE consumption_kwh IS NOT NULL`
        ...
        for (const row of electricityRowsResult.rows) {
            const emissions = calculateElectricityEmissions(parseFloat(row.consumption_kwh), electricityFactor);
